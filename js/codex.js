 ...
 *   形象在 monsters.js（矢量画），元素图标在 icons.js（矢量路径），
...
;(function (root) {
  ...
  /* ------------------------------------------------------------------ */
  /*  2) 档案文案                                                        */
  /*  role  是定位标签（格子与详情页都显示）
   *  tag   一句话概括
   *  lines 详情页/弹窗里的「要点」——每行控制在 20 个汉字以内，
   *        超了 layout-check 的「弹窗文字不越框」会直接报出来。
   *  howto 「打法」——这只怪**怎么应对**。加这一栏是因为实测反馈：
   *        第 5 波那只首领血厚、护甲高，光看数值根本不知道该干什么
   *        （人的第一反应是再加两座同样的塔，方向正好反了）。
   *        文案里尽量不写死数字 —— 会过期的数字由 C.enemy() 现读 CFG 拼
   *        （见 counter），这里只写方法论。
   * ------------------------------------------------------------------ */
  var ENEMY = {
    drifter: {
    ...
    boss: {
      role: '首领',
      tag: '第 5/10/15/20 波 · 高护甲',
      lines: [
        '血量与护甲都远高于杂兵，平砍会被削掉大半',
        '靠近时压制半径内的塔会脱离共振网络'
      ],
      howto: [
        '让异元素贴边组反应 —— 反应伤害无视护甲',
  ...
  C.enemy = function (key) {
    ...
    if (!d) return null;
    var info = ENEMY[key] || { role: '', tag: '', lines: [], howto: [] };
    return {
      key: key, def: d, name: d.name, color: d.color, role: info.role,
      tag: info.tag, lines: info.lines, howto: info.howto || [],
      /* 「数值标注」：把所有会随平衡改动而变的数字拼在这一条里，
       * 现读 CFG —— 文案表里就不必写死「护甲 6」这种迟早过期的东西。
       * 首领额外带上压制半径（它是这只怪唯一的机制数字）。 */
      counter: (function () {
        var parts = ['护甲 ' + d.armor];
        if (d.boss && d.suppress) parts.push('压制半径 ' + d.suppress);
        if (d.towerDps) parts.push('拆塔 ' + d.towerDps + '/秒');
        if (d.phaseCycle) parts.push('相位 ' + d.phaseCycle + 's 一轮');
        if (d.healRange) parts.push('治疗半径 ' + d.healRange);
        return parts.join(' · ');
      })(),
